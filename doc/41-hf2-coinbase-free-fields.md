...
> header (slot[4]=PoW nonce, slot[5]=fork-signal commitment), which is what frees
...
> requires empty; recipient==address kept) + digest readers (nonce from slot[4], signal from slot[5],
...
TWO consensus payloads at once (see fork.py, digest_tx.py):

```
openfield = [FORK2_SIGNAL "hf2"]  +  <PoW nonce>
```
...
So a miner literally cannot leave `openfield` empty without breaking PoW verification and fork
signaling. "Nothing enforced" is impossible while these ride in `openfield`.
...
| `[5] public_key` | b64 pubkey | **mining commitment** = optional `"hf2"` fork-readiness vote |
| `[6] operation`  | op / "0" | **free-form, optional, uncapped miner data** |
| `[7] openfield`  | nonce+signal | **free-form, optional, uncapped miner data** |
...
The nonce and the signal commitment MUST stay committed to the post-fork block hash (else a miner
...
coinbase v2 pre-image =
  ...
  | commitment(lp1)         # from slot[5]: ["hf2"]
...
`miner.py` / `pool/optipoolware.py`: post-fork, write `nonce` → signature slot and the optional
`"hf2"` signal → public_key slot; leave `operation`/`openfield` empty unless the operator
...
- One fork only (hf2); no second signal. Pre-fork serialization, PoW, and fork-signal reads are
  byte-for-byte unchanged (characterization-locked).
- Nonce + signal commitment remain in the block-hash pre-image → PoW keeps its teeth.
