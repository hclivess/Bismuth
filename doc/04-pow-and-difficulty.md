...
that seeds the lookup file) and `difficulty.py` (retarget). `digest.py` calls `check_block()` for
every block; `difficulty()` is recomputed after each block.
...
q_received_timestamp, q_db_timestamp_last, peer_ip='N/A', app_log=None)`** returns the `diff0` to
store and raises `ValueError` if the nonce fails. Acceptance:
