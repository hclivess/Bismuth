...
def _fresh_ledger():
    ...
    cur = conn.cursor()
    cur.execute("CREATE TABLE transactions (block_height, signature)")
...
def test_migration_sets_version_creates_indexes_and_is_idempotent():
    ...
    assert db_migrations.run(cur, migs) == 1
    idx = _indexes(cur)
...
def test_old_sqlite_skips_the_substr_index():
    conn, cur = _fresh_ledger()
    db_migrations.run(cur, db_migrations.ledger_migrations(old_sqlite=True))
    idx = _indexes(cur)
