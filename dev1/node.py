        ...
        while True:
            try:
                ...
                if data == "blockheight":
                    ...
                    block_height_received = data
                    print "Received block height to compare: "+(block_height_received)
                #send block height, receive block height
                    
                    if block_height_received > db_block_height:                    
                        #send block height, receive block height
                        #node has higher block, start receiving txs from it
                        conn = sqlite3.connect('ledger.db')
                        c = conn.cursor()                
                        ...
                        connection.sendall ("My latest txhash is as follows")
                        time.sleep(0.1)
                        ...
                        if data == "Block found":
                            ...
                            if received_txhash == hashlib.sha224(str(received_transaction) + str(received_signature) +str(txhash_db)).hexdigest(): #new hash = new tx + new sig + old txhash
                                ...
                                #connection.sendall("Sync finished")
                                #update local db with received tx                    
